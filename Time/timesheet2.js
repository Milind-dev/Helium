...
function startTimer() {
    ...
    html += ` <div class="timer__cardlistItem">
            <div class="timercard__listItem__left">
                <div >
                    <h5>${dropDownProjectNameOption}</h5><span></span>
                </div><p>${dropDownProjectTaskOption}</p>
            </div>
            <div class="timercard__listItem__right">
                <h3 id = "showTimer"></h3>
                <button id = "statingTime" onClick = "startAgain()" ><img src="./dynamic_clock.gif" alt="running__clock"> Start</button>
                <button id = "timerStop" onClick = "changeButton()"><i class="fa fa-clock-o" aria-hidden="true"></i> Stop</button>
